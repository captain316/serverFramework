 ...
 */
#ifndef __CAPTAIN_TCP_SERVER_H__
#define __CAPTAIN_TCP_SERVER_H__
...
public:
     ...
     */
    TcpServer(captain::IOManager* worker = captain::IOManager::GetThis()
              ,captain::IOManager* io_woker = captain::IOManager::GetThis()
     ...
     * @return 返回是否绑定成功
     */
    virtual bool bind(captain::Address::ptr addr, bool ssl = false);
    ...
    //加载 SSL/TLS 证书和密钥文件，用于加密和解密安全连接。
